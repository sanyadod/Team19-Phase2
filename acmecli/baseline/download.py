...
def _fetch_metadata(artifact_type: str, artifact_id: str) -> dict:
    ...
    try:
        ...
        logger.error("DynamoDB error code: %s, error message: %s, table: %s", 
    ...
    item = resp.get("Item")
    if not item:
        logger.error("Artifact NOT FOUND in DynamoDB: artifact_type=%s, artifact_id=%s, table=%s", 
                    artifact_type, artifact_id, META_TABLE.table_name)
    ...
    if item.get("artifact_type") != artifact_type:
        logger.error("Artifact TYPE MISMATCH: expected=%s, found=%s, artifact_id=%s, item_keys=%s",
                    artifact_type, item.get("artifact_type"), artifact_id, list(item.keys()))
...
def _generate_presigned_url(bucket: str, key: str) -> str:
    ...
    try:
        ...
        if not url or not url.startswith('https://'):
            logger.error("Presigned URL GENERATION FAILED: invalid URL format, bucket=%s, key=%s, url=%s", 
                        bucket, key, url)
            logger.error("URL type: %s, URL length: %s, starts with https: %s", 
                        type(url), len(url) if url else 0, url.startswith('https://') if url else False)
            abort(500, description="The artifact storage encountered an error.")
        return url
    except ClientError as e:
        logger.error("S3 presigned URL generation FAILED: bucket=%s, key=%s, error=%s", bucket, key, e, exc_info=True)
        logger.error("S3 error code: %s, error message: %s, error response: %s",
                    e.response.get("Error", {}).get("Code", "Unknown"),
                    e.response.get("Error", {}).get("Message", "Unknown"),
                    e.response)
        abort(500, description="The artifact storage encountered an error.")
    except Exception as e:
        logger.error("Unexpected error generating presigned URL: bucket=%s, key=%s, error=%s", 
...
def list_artifacts():
    ...
    if queries is None:
        logger.error("Request body is None or not JSON. Content-Type: %s, Data length: %d, Data: %s",
                    request.content_type, len(request.data) if request.data else 0, request.data)
        abort(400, description="There is missing field(s) in the artifact_query or it is formed improperly, or is invalid.")
    
    if not isinstance(queries, list) or not queries:
        logger.error("Invalid queries format: type=%s, value=%s, is_list=%s, is_empty=%s", 
                    type(queries), queries, isinstance(queries, list), not queries if isinstance(queries, list) else "N/A")
        abort(400, description="There is missing field(s) in the artifact_query or it is formed improperly, or is invalid.")
    ...
    try:
        ...
        logger.error("Error type: %s, traceback: %s", type(e).__name__, e.__traceback__)
        abort(500, description="The artifact storage encountered an error.")
...
def get_artifact(artifact_type: str, artifact_id: str):
    ...
    # Check if download_url is requested via include query parameter
    ...
    if include_download_url:
        try:
            ...
            if not presigned_url or not isinstance(presigned_url, str):
                logger.error("Invalid presigned URL: type=%s, value=%s, bucket=%s, key=%s", 
                            type(presigned_url), presigned_url, bucket, key)
                abort(500, description="The artifact storage encountered an error.")
            
            if not presigned_url.startswith('https://'):
                logger.error("Presigned URL does not start with https://: url=%s, bucket=%s, key=%s", 
                            presigned_url, bucket, key)
                abort(500, description="The artifact storage encountered an error.")
            
            data["download_url"] = presigned_url
        except Exception as e:
            logger.error("Failed to generate presigned URL: bucket=%s, key=%s, error=%s", 
                        bucket, key, e, exc_info=True)
            abort(500, description="The artifact storage encountered an error.")
    ...
    if "name" not in body["metadata"] or "id" not in body["metadata"] or "type" not in body["metadata"]:
    ...
    if include_download_url:
        ...
        if not body["data"]["download_url"] or not body["data"]["download_url"].startswith('https://'):
            logger.error("Invalid download_url: url=%s, type=%s, starts_with_https=%s", 
                        body["data"]["download_url"], 
                        type(body["data"]["download_url"]),
                        body["data"]["download_url"].startswith('https://') if body["data"]["download_url"] else False)
            logger.error("Full response body: %s", body)
            abort(500, description="The artifact storage encountered an error.")

    if include_download_url:
        logger.info("GET /artifacts/%s/%s: success, id=%s, download_url_len=%d", 
                    artifact_type, artifact_id, db_artifact_id, len(data["download_url"]))
    else:
        logger.info("GET /artifacts/%s/%s: success, id=%s, download_url not requested", 
                    artifact_type, artifact_id, db_artifact_id)
